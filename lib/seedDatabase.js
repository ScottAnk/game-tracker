...
const seedGames = async () => {
  ...
  await Promise.all(games).then((games) => {
    ...
    const groupOneGames = remainderGameIds.slice(0, 3)
    const groupTwoGames = remainderGameIds.slice(3, 6)
    const groupThreeGames = remainderGameIds.slice(6)
    return Promise.all([
      ...
      Collection.findOneAndUpdate(
        { ownerId: users[2]._id, title: 'My Games' },
        { games: remainderGameIds }
      ).exec(),
      Collection.findOneAndUpdate(
        { ownerId: users[2]._id, title: 'Party Games' },
        { games: groupOneGames }
      ).exec(),
      Collection.findOneAndUpdate(
        { ownerId: users[2]._id, title: 'Heavy Games' },
        { games: groupTwoGames }
      ).exec(),
      Collection.findOneAndUpdate(
        { ownerId: users[2]._id, title: 'Card Games' },
        { games: groupThreeGames }
      ).exec(),
    ])
  })
